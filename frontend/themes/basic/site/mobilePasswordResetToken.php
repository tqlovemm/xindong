    ...
    <div class="row center-block" style="padding: 20px 0;max-width: 400px;">
        <div class="" style="background-color: #fff;padding: 10px 20px;">
            ...
            <div class="form-group">
                <?= Html::submitButton('确定', ['class' => 'btn btn-primary',  'style'=>'border-color:#E83F78;background-color:#E83F78;width:100%;padding: 5px 0;font-size:20px;','name' => 'login-button']) ?>
            </div>
            <?php ActiveForm::end(); ?>
        </div>
    </div>
...
<script>
    ...
    function sendCode(obj){
        ...
        var mobile = $("#passwordresetmobileform-cellphone").val();    //检查手机是否合法
        if(isPhoneNum(mobile)){
            var exists_result = dbCheckMobileExists(site+'check-mobile-exists',{"mobile":mobile,"type":"reset"});
            if(exists_result) {
                var send = doPostBack(site + 'send-login-code', {'mobile': mobile,"type":"reset"});
                if(send){
    ...
    function isPhoneNum(obj){
        ...
        if(!myreg.test(obj)){
            alert('请输入有效的手机号码！');
            $("#passwordresetmobileform-cellphone").focus();
            return false;
        }else{
            return true;
        }
