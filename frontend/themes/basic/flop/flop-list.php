...
<div id="narrow" style="color:#fff;font-size: 26px;line-height: 50px;padding-top: 40%;text-align: center;">
    <h1>分享给微信客服:</h1>
    <h3><?=$girlPhoto[0]['name']?></h3>
</div>
<header>
    <div class="header">
        <a href="javascript:history.back();"><span><img src="<?=Yii::getAlias('@web')?>/images/iconfont-fanhui.png"></span></a>
        <h2><?=$this->title?></h2>
        <a href="<?=Url::to(['/flop/teach'])?>" style="right:3%;top:0;font-size:18px;line-height: 44px;">教程</a>
    </div>
</header>
...
<div id="masonry" class="container-fluid" style="padding: 10px;">
        ...
        <?php foreach($model as $item):?>
            <div data-title="<?=$item['id']?>" class="box priority-img"  style="position: relative;background-color: #fff;padding:10px;">
                <img style="box-shadow: 3px 3px 5px #adadad;border-radius: 3px;" src="<?=$item['path']?>">
                <h5 style="margin-bottom: 0;">编号：<?=$item['number']?></h5>
                <h5 style="margin-bottom: 0;">地区：<?=$item['area']?></h5>
...
<div class="text-center" style="margin-top: 20px;margin-bottom: 20px;"><a class="btn btn-danger"  style="color:white;padding:4px 80px;width: 320px;height: 40px;line-height: 30px;" href="<?=Url::to(['/flop/clear-flop'])?>" data-confirm="确认清空吗">清空返回重选</a></div>
