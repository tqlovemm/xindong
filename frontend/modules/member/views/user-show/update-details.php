...
<div style="position: fixed;bottom:0;left:0;width: 100%;padding: 0;">
    ...
    <a <?php if(!Yii::$app->user->isGuest): if(Yii::$app->user->identity->groupid<intval($model['groupid'])):?>href="<?=\yii\helpers\Url::to(['pay-type','id'=>$model['id']])?>"<?php else:?>data-confirm="您无需升级"<?php endif; else:?> onclick="upgrade({'upgrade_id':<?=$update_id?>,'group_id':<?=$update_id['groupid']?>,'original_price':<?=$price?>,'preferential_price':<?=$need_price?>})" <?php endif;?> style="float: left;width: 70%;background-color: #000;padding:10px;font-size: 24px;color:#FFA72A;text-align: center;line-height: 30px;display: block;border-top: 1px solid #000;">
        升级会员 >>>
    </a>
</div>
