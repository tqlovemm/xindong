            ...
            $html =<<<HTML
              <div class="media">
                ...
                <div class="row">
                  ...
                  <div class="col-xs-4 text-center">
                    <span class="block font-14">{$userData['thread_count']}</span><br>
...
HTML;
        ...
        $query = (new Query)->select('*')
            ->from('{{%home_photo}}')
            ->where('created_by=:created_by', [':created_by' => $model->id])
            ->orderBy('created_at DESC');

        $posts = Yii::$app->tools->Pagination($query);
        $photo = $posts['result'];
        $photo_num = count($photo)>3?3:count($photo);


        $count_note = Yii::$app->db->createCommand('select SUM(note) from {{%forum_thread}} where user_id='.$model->id)->queryScalar();
        $note_num = empty($count_note)?0:$count_note;
