...
class MessageController extends BaseController
{
    ...
    public function behaviors()
    {
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['mention', 'inbox', 'outbox', 'create', 'view', 'update', 'upload','delete'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
    ...
    public function actionView($id)
    {
        ...
        $query->select('username')
            ->from('{{%user}}')
            ->where(['id' => $model->sendfrom]);
        $pages2 = Yii::$app->tools->Pagination($query);

        return $this->render('view', [
            'model' => $model,
            'count' => $this->getMessageCount(),
            'result1'=>$pages['result'],
            'result2'=>$pages2['result'],
        ]);
    }
     ...
     */
     public function actionUpdate()
     {
         ...
         if(isset($_GET['sendFrom'])){

             $from = $_GET['sendFrom'];
             return $this->redirect('/index.php/user/message/create?from='.$from);
         }
    ...
    public function actionDelete($id)
    {
        ...
        return $this->redirect(['/user/message/inbox']);
    }
