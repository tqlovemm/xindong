...
<div class="nav-tabs-top">
    ...
    <ul id="myTab" class="nav nav-tabs nav-tabs-man" style="margin-bottom: 10px;background-color: #fff;">
        ...
        <li class="active col-xs-4" style="">
...
<div id="myTabContent" class="tab-content" style="padding-bottom: 100px;min-height: 350px;">
    <div class="tab-pane fade in active" id="man">
        <ul class="wall">
            ...
            <li class="article">
                <h5 style="color:gray;">参赛编号：<?=$woman['id']?>号</h5>
                ...
                <a class="weicaht-note" data-lightbox="d" data-title="请关注微信订阅号进行投票" href="/images/weixin/8505.jpg">
                    <span class="glyphicon glyphicon-thumbs-up"></span><span class="note-count note-padding"> <?=$woman['num']?></span>
                </a>
                <?php else:?>
                <div class="weicaht-note">
                    <?php $model = $good::findOne(['da_id'=>$woman['id'],'sayGood'=>$session->get('openId')])?>
                    <span class="glyphicon glyphicon-thumbs-up <?php if($model) echo 'abd-active';?>"  onclick="vote_notes(<?=$woman['id']?>,'<?=$session->get('openId')?>',this )">
                    </span><span class="note-count note-padding"> <?=$woman['num']?></span>
                </div>
                <?php endif;?>
            </li>
        <?php endforeach;?>
