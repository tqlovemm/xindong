...
$this->registerCss("
    ...
    .weicaht-note{padding:3px 0;display:block;font-size: 20px;text-align:center;background-color:#22222E;color:#C1C1C1;border-radius: 3px;margin-top:3px;}
...
<div class="nav-tabs-top">
    ...
    <ul id="myTab" class="nav nav-tabs nav-tabs-man" style="margin-bottom: 10px;background-color: #fff;">
        ...
        <li style="border-right: 1px solid #eee;border-left: 1px solid #eee;" class="col-xs-4">
    ...
    <div class="tab-pane fade in active" id="man">
        <ul class="wall">
            <?php foreach($model as $man):?>
                <li class="article">
                    <h5 style="color:gray;">参赛编号：<?=$man['id']?>号</h5>
                    ...
                    <div class="note-count note-padding"> </div>
                        ...
                        <a class="weicaht-note" data-lightbox="d" data-title="请关注微信订阅号进行投票" href="/images/weixin/8505.jpg">
                            <span class="glyphicon glyphicon-thumbs-up"></span> <span class="note-count note-padding"><?=$man['num'];$subscribe?></span>
                        </a>
                        <?php else:?>
                        <div class="weicaht-note">
                            <?php $model = $good::findOne(['da_id'=>$man['id'],'sayGood'=>$session->get('openId')]);?>
                            <span class="glyphicon glyphicon-thumbs-up <?php if($model) echo 'abd-active';?>" onclick="vote_notes(<?=$man['id']?>,'<?=$session->get('openId')?>',this)"></span>
                            <span class="note-count note-padding"><?=$man['num']?></span>
                        </div>
                        <?php endif;?>
                </li>
            <?php endforeach;?>
