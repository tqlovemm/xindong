...
$this->registerCss("
    ...
    .wall-column{padding-bottom:0 ;margin-bottom:80px;  !important;margin}
    ...
    #topTab > .woman.active > a, .nav-tabs > .woman.active > a:hover, .nav-tabs > .woman.active > a:focus{color:#E8437B;}
    ...
    .weicaht-note{padding:3px 0;display: block;font-size: 20px;border-radius:3px;background-color:#22222E;text-align:center;color:#C1C1C1;margin-top:3px;}
...
<div class="nav-tabs-top">
    ...
    <ul id="myTab" class="nav nav-tabs nav-tabs-woman" style="margin-bottom: 10px;background-color: #fff;">
        ...
        <li class="active col-xs-4" style="border-right: 1px solid #eee;border-left: 1px solid #eee;">
...
<div id="myTabContent" class="tab-content" style="padding-bottom: 100px;min-height: 350px;">
    <div class="tab-pane fade in active" id="man">
        <ul class="wall">
            <?php foreach($model as $woman):?>
            <li class="article">
                <h5 style="color:gray;text-align: left">参赛编号：<?=$woman['id']?>号</h5>
                <a href="center?id=<?=$woman['id']?>">
                    <img src="<?=$woman['img']['thumb']?>"/>
                </a>
                <?php if($subscribe !== 1):?>
                    <div style="padding: 0;margin: 0;font-size: 14px;">总票数：<span class="note-count note-padding"><?=$woman['num'];?></span></div>
                ...
                <div class="weicaht-note" style="color: #fff">
                    <?php $model = $good::findOne(['da_id'=>$woman['id'],'sayGood'=>$session->get('openId')])?>
                    <span class="glyphicon glyphicon-heart <?php if($model) echo 'abd-active';?>"  onclick="vote_notes(<?=$woman['id']?>,'<?=$session->get('openId')?>',this )">
                    </span> 投票
                </div>
                <?php endif;?>
            </li>
...
<div class="text-center" style="margin-right: 3%;"><?=LinkPager::widget(['pagination'=>$pages,'firstPageLabel'=>"首页",'lastPageLabel'=>"末页",'prevPageLabel'=>"上页",'nextPageLabel'=>"下页","maxButtonCount"=>0])?>  </div>
