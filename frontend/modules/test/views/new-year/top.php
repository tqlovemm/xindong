            ...
            foreach($woman as $key=>$list):
                ...
                <div class="vote-top-box">
                    ...
                    <div class="row" style="padding: 0px;margin:0; text-align: center">
                        <?php if($subscribe !== 1):?>
                            <div>总票数：<span class="note-count note-padding"><?=$list['num']?></span></div>
                            <a class="weicaht-note" style="color: #fff" data-lightbox="d" data-title="请关注微信订阅号进行投票" href="/images/weixin/8505.jpg">
                                <span class="glyphicon glyphicon-heart" style="padding-top: 3px;"></span> 投票
                            </a>
                        <?php else:?>
                            <div>总票数：<span class="note-count note-padding"><?=$list['num']?></span></div>
                            <div class="weicaht-note"  style="color: #fff">
                                <?php $model = $good::findOne(['da_id'=>$list['id'],'sayGood'=>$session->get('openId')]);?>
                                <span class="glyphicon glyphicon-heart <?php if($model){echo 'abd-active';}?>" onclick="vote_notes(<?=$list['id']?>,'<?=$session->get('openId')?>',this)"> </span>
                                 投票
                            </div>
                        <?php endif;?>
                    </div>
                   ...
                   <!-- <img class="vote-top-icon" src="/images/vote/man<?/*=$key+1*/?>.png">-->
                    ...
                    <div class="row" style="padding: 0px;margin:0;text-align: center">
                        <?php if($subscribe !== 1):?>
                            <div>总票数：<span class="note-count note-padding"><?=$list['num']?></span></div>
                            <a class="weicaht-note" style="color: #fff" data-lightbox="d" data-title="请关注微信订阅号进行投票" href="/images/weixin/8505.jpg">
                                <span class="glyphicon glyphicon-heart" style="padding-top: 3px;"></span> <span class="note-count note-padding"><?=$list['num']?></span>
                            </a>
                        <?php else:?>
                            <div>总票数：<span class="note-count note-padding"><?=$list['num']?></span></div>
                            <div class="weicaht-note"  style="color: #fff">
