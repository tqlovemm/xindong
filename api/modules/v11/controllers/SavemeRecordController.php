...
class SavemeRecordController extends ActiveController {
    ...
    public function actionCreate(){
        ...
        $apply = (new Query())->select('status')->from('{{%saveme_apply}}')->where(['saveme_id'=>$saveme_id,'apply_uid'=>$boy_id])->orderBy('created_at desc')->one();
        if($recordres && $recordres['created_id'] == $girl_id && $saveme['end_time'] > $time && $apply['status'] == 1){
