...
class MessageController extends ActiveController
{
    ...
    public function actionView($id){
        ...
        $info = (new Query())
            ...
            ->where(" to_id = {$id} and from_id <> {$id} and is_read = 1 ")
            //->orwhere("w.user_id={$id} and to_id <> {$id} and from_id <> {$id} and is_read = 1 ")
            ->count();
        if($info){
            Response::show('200','操作成功',$info);
        }else{
            Response::show('201','操作失败','没有新通知');
        }
    }
