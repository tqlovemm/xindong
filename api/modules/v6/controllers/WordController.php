...
class WordController extends ActiveController
{
    ...
    public function actionView($id)
    {
        ...
        if($message){
            ...
            if($data){
                ...
                //Yii::$app->db->createCommand("update pre_app_push set is_read=0 where message_id in ({$data})")->execute();
                //Yii::$app->db->createCommand("delete from pre_app_message where id in ({$data})")->execute();
    ...
    protected function getAge($id){
        $brithdate = (new Query())->select('birthdate')->from('{{%user_profile_copy}}')->where(['user_id'=>$id])->one();
        $age = date('Y-m-d',time())-$brithdate['birthdate'];
        return $age;
    }
