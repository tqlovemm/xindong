...
class Wechatclass
{
	...
	// 企业支付
	const MKT_PAY_URL = 'https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers'; //语义理解
...
<Nonce><![CDATA[%s]]></Nonce>
     ...
     * @return boolean
     */
    public function mkt_pay($openid=array(),$user=array()) {
        $data = array();
        if (count($openid) > 0)
            $data['openid'] = $openid;
        if (count($user) > 0)
            $data['username'] = $user;
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::MKT_PAY_URL, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
