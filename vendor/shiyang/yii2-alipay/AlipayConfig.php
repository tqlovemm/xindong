...
//合作身份者ID，签约账号，以2088开头由16位纯数字组成的字符串，查看地址：https://b.alipay.com/order/pidAndKey.htm
        $this->alipay_config['partner']		= \Yii::$app->params['partner'];

//收款支付宝账号，以2088开头由16位纯数字组成的字符串，一般情况下收款账号就是签约账号
        $this->alipay_config['seller_id']	= $this->alipay_config['partner'];

// MD5密钥，安全检验码，由数字和字母组成的32位字符串，查看地址：https://b.alipay.com/order/pidAndKey.htm
        $this->alipay_config['key']			= \Yii::$app->params['key'];

// 服务器异步通知页面路径  需http://格式的完整路径，不能加?id=123这类自定义参数，必须外网可以正常访问
        $this->alipay_config['notify_url'] = \Yii::$app->params['notifyUrl'];

// 页面跳转同步通知页面路径 需http://格式的完整路径，不能加?id=123这类自定义参数，必须外网可以正常访问
        $this->alipay_config['return_url'] = \Yii::$app->params['returnUrl'];
//$alipay_config['return_url'] = "http://localhost/member-ship/jiecao-coin/create_direct_pay_by_user-PHP-UTF-8/return_url.php";
