...
class Recharge extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['title','content','url','number','avatar'], 'string', 'max' => 256]
        ];
    }
    ...
    public static function getUrl($type){
        ...
        $query = ArrayHelper::map(Recharge::findAll(["status"=>2,'title'=>$type]), 'number', 'number');
        return $query;
    ...
    public function getContent($id){
        ...
        return Yii::$app->db
            ->createCommand("SELECT number,title,content,url,created_at,cover_id FROM {{%weekly}} WHERE id='$id'")
            ->queryOne();
    ...
    {
        $config = [
            'savePath' => Yii::getAlias('@backend').'/web/uploads/recharge/', //存储文件夹
            'maxSize' => 2048 ,//允许的文件最大尺寸，单位KB
            'allowFiles' => ['.gif' , '.png' , '.jpg' , '.jpeg' , '.bmp'],  //允许的文件格式
        ];
        $up = new Uploader2("file", $config, 'recharge'.$this->id,true);

        $save_path =  Yii::getAlias('@web/uploads/recharge/') . $this->created_by.'/watermark/';
        $info = $up->getFileInfo();
