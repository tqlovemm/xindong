...
/**
 * This is the model class for table "pre_smadmin_member".
 *
 ...
 *
 * @property SmadminMemberText $memberText
 */
class SmadminMember extends \yii\db\ActiveRecord
{
    public $record_type;
    /**
     * @inheritdoc
    ...
    {
        $member = SmadminMember::findOne(['number'=>$this->number]);
        if($this->isNewRecord) {
            if (!empty($member)) {
    ...
    {
        return $this->hasMany(SmadminMemberText::className(), ['member_id' => 'member_id'])->joinWith('memberFiles')->where(['type'=>$type])->orderBy('pre_smadmin_member_text.created_at desc');
    }
