...
/**
 * This is the model class for table "pre_smadmin_member_text".
 *
 ...
 * @property SmadminMemberFiles $memberFiles
 */
class SmadminMemberText extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    {
        return 'pre_smadmin_member_text';
    }
    ...
    public function upload()
    {
        ...
        /*$config = [
            'savePath' => Yii::getAlias('@backend').'/web/uploads/smadmin/', //存储文件夹
            'maxSize' => 4096 ,//允许的文件最大尺寸，单位KB
            'allowFiles' => ['.gif' , '.png' , '.jpg' , '.jpeg' , '.bmp'],  //允许的文件格式
        ];
        $up = new Uploader("file", $config, 'smadmin'.$this->text_id);

        $save_path =  Yii::getAlias('@web/uploads/smadmin/') . Yii::$app->user->id.'/';
        $info = $up->getFileInfo();*/

        //存入数据库
        $model = new SmadminMemberFiles();
    ...
    {
        return $this->hasOne(SmadminMember::className(), ['member_id' => 'member_id']);
    }    /**
     * @return \yii\db\ActiveQuery
     */
    public function getMemberFiles()
    {
        return $this->hasMany(SmadminMemberFiles::className(), ['text_id' => 'text_id']);
    }
}
