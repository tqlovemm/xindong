            ...
            if($model->sex==0):
                ...
                foreach ($flops as $key=>$flop):?>
                    <div class="rizhi" style="box-shadow: 0 5px 4px #E5E4E4;">
                        ...
                        <div class="row" style="border-bottom: 1px solid #eee;">
                            <div class="col-md-12">
                                <?=date('Y年m月d日',$flop['created_at'])?>，该男生会员对编号为:<?=$flop['like_id']?>的女生会员进行觅约报名，该条记录由<?=$flop['user_id']?>创建生成
                            </div>
                        </div>
                        <div class="row">
                            <?php
                            $dating = $date::find()->where(['number'=>$flop['like_id']])->asArray()->all();
                ...
                foreach ($flops as $key=>$flop):?>
                    <div class="rizhi" style="box-shadow: 0 5px 4px #E5E4E4;">
                        ...
                        <div class="row" style="border-bottom: 1px solid #eee;">
                            <div class="col-md-12">
                                <?=date('Y年m月d日',$flop['created_at'])?>，该女生被编号为:<?=\backend\models\User::getNumber($flop['user_id'])?>的男生会员进行密约报名，该条记录由<?=$flop['like_id']?>创建生成
                            </div>
                        </div>
                        <div class="row">
                            <?php
                            $dating = $date::find()->where(['number'=>\backend\models\User::getNumber($flop['user_id'])])->asArray()->all();
                            foreach ($dating as $list):?>
                                <div class="col-sm-1">
