...
<div class="thirth-files-form">
    ...
    <?= $form->field($model, 'flop_id')->dropDownList($area)->label('地区') ?>
    <?= $form->field($model, 'flop_id2')->dropDownList($area, ['prompt'=>'---请选择地区---'])->label('地区二') ?>
    <?= $form->field($model, 'flop_id3')->dropDownList($area, ['prompt'=>'---请选择地区---'])->label('地区三') ?>
