<?php

namespace backend\modules\dating\models;
use Yii;
use yii\db\Query;
use backend\components\Uploader2;
use yii\db\ActiveRecord;

/**
 * This is the model class for table "{{%heartweek}}".
 *
 * @property string $id
 * @property string $title
 * @property string $content
 * @property string $url
 * @property string $number
 * @property integer $cover_id
 * @property string $avatar
 * @property integer $created_at
 * @property integer $updated_at
 * @property integer $created_by
 * @property string $enable_comment
 * @property string $file
 * @property integer $status
 */
class HeartWeek extends ActiveRecord
{

    const TYPE_PUBLIC = 0;
    const TYPE_PASSWORD = 1;
    const TYPE_QUESTION = 2;
    const TYPE_PRIVATE = 3;

    const COVER_NONE = 0;

    /**
     * @inheritdoc
     */
    public static function tableName()
    {
        return '{{%heartweek}}';
    }

    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['title','content'], 'required'],
            [['cover_id', 'created_at', 'updated_at', 'created_by',  'status'], 'integer'],
            [['title','content','url','number','avatar'], 'string'],
            ['file', 'file', 'extensions' => 'mp3'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function attributeLabels()
    {
        return [
            'id' => Yii::t('app', 'ID'),
            'number' => '妹子编号',
            'title' => '标题',
            'content' => '内容描述',
            'url'=>'期刊数（如：第2期）',
            'avatar'=>'密约头像',
            'cover_id' => Yii::t('app', 'Cover ID'),
            'created_at' => Yii::t('app', '创建时间'),
            'updated_at' => Yii::t('app', '更新时间'),
            'created_by' => Yii::t('app', '创建人'),
            'file' => '附件（类型为声色的时候添加mp3）',
            'status' => Yii::t('app', '状态码'),

        ];
    }

    public function getPhotoCount()
    {
        return Yii::$app->db
            ->createCommand("SELECT count(*) FROM {{%heartweek_content}}  WHERE album_id={$this->id}")
            ->queryScalar();
    }

    public function getCPhoto($id)
    {
        $avatar = Yii::$app->db
            ->createCommand("SELECT avatar FROM {{%heartweek}} WHERE id=$id")
            ->queryScalar();
        if(!empty($avatar)){
            return $avatar;
        }else{
            return Yii::getAlias('@web') . '/images/pic-none.png';
        }
    }

    /**
     * This is invoked before the record is saved.
     * @return boolean whether the record should be saved.
     */
    public function beforeSave($insert)
    {
        if (parent::beforeSave($insert)) {
            if ($this->isNewRecord) {
                $this->created_by = Yii::$app->user->id;
                $this->created_at = time();
                $this->updated_at = time();
                $this->cover_id = self::COVER_NONE;
            }

            return true;
        } else {
            return false;
        }
    }


    public function getArea($id){

        return Yii::$app->db
            ->createCommand("SELECT title FROM {{%heartweek}} WHERE id=$id")
            ->queryOne();
    }
    public function getPhotos()
    {
        $query = new Query;
        $query->select('id, name, path')
            ->from('{{%heartweek_content}}')
            ->where('album_id=:id', [':id' => $this->id]);
        $photos = Yii::$app->tools->Pagination($query);
        return [
            'photos' => $photos['result'],
            'pages' => $photos['pages']
        ];
    }
    public function getPhoto($id)
    {
        $query = new Query;
        $query->select('id, name, path')
            ->from('{{%heartweek_content}}')
            ->where('album_id=:id', [':id' => $id]);
        $photos = Yii::$app->tools->Pagination($query);
        return [
            'photos' => $photos['result'],
            'pages' => $photos['pages']
        ];
    }
    public function getComment($id)
    {
        return Yii::$app->db
            ->createCommand("SELECT id,content,likes,created_at FROM {{%heartweek_comment}} WHERE weekly_id='$id' order by id desc")
            ->queryAll();
    }

    public function getContent($id){

        return Yii::$app->db
            ->createCommand("SELECT number,title,content,url,created_at,cover_id FROM {{%heartweek}} WHERE id='$id'")
            ->queryOne();


    }

    public function getUser()
    {
        return Yii::$app->db
            ->createCommand("SELECT username, avatar FROM {{%user}} WHERE id={$this->created_by}")
            ->queryOne();
    }

    /**
     * 取得封面图片的地址
     * 本来只要一个相册id也是可以的，但为减少查询，故同时要 $id, $cover_id
     * @param integer $id 相册id
     * @param integer $cover_id 封面id
     */
    public function getCoverPhoto($id = null, $cover_id = null)
    {
        if ($id == null) {
            if ($this->cover_id == self::COVER_NONE) {
                if ($this->photoCount == 0) {
                    return '/images/pic-none.png';
                } else {
                    $path = $this->photos[0]['path'];
                }
            } else {
                $path = Yii::$app->db
                ->createCommand('SELECT path FROM {{%heartweek_content}} WHERE id='.$this->cover_id)
                ->queryScalar();
            }
        } else {
            if ($cover_id == self::COVER_NONE) {
                $path = Yii::$app->db
                    ->createCommand('SELECT path FROM {{%heartweek_content}} WHERE album_id='.$id)
                    ->queryScalar();
                if (empty($path)) {
                    return Yii::getAlias('@web') . '/images/pic-none.png';
                }
            } else {
                $path = Yii::$app->db
                    ->createCommand('SELECT path FROM {{%heartweek_content}} WHERE id='.$cover_id)
                    ->queryScalar();
            }
        }
        return $path;
    }

    /**
     * 处理图片的上传
     */
    public function upload()
    {
        $config = [
            'savePath' => Yii::getAlias('@backend').'/web/uploads/heartweek/', //存储文件夹
            'maxSize' => 4048 ,//允许的文件最大尺寸，单位KB
            'allowFiles' => ['.gif' , '.png' , '.jpg' , '.jpeg' , '.bmp'],  //允许的文件格式
        ];
        $up = new Uploader2("file", $config, 'heartweek'.$this->id);

        $save_path =  Yii::getAlias('@web/uploads/heartweek/') . $this->created_by.'/';
        $info = $up->getFileInfo();

        //存入数据库
        Yii::$app->db->createCommand()->insert('{{%heartweek_content}}', [
            'name' => $this->title,
            'path' => 'http://13loveme.com:82/'.$save_path . $info['name'], //存储路径
            'store_name' => $info['name'], //保存的名称
            'album_id' => $this->id,
            'created_at' => time(),
            'created_by'=>Yii::$app->user->id,
        ])->execute();
    }
    public function uploads()
    {

        if ($this->validate()) {

            Yii::setAlias('@upload', '@backend/web/uploads/heartweek/mp3/');

            $path = time().rand(999,10000) . '.' . $this->file->extension;

            $this->file->saveAs(Yii::getAlias('@upload').$path );

            return 'http://13loveme.com:82/uploads/heartweek/mp3/'.$path;

        } else {

            return false;
        }
    }
}
