...
- Enh: Custom side menu via `backend\modules\rbac\Module::items` (mdmunir).
...
- Chg: Can customize return of `backend\modules\rbac\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `backend\modules\rbac\components\BizRule` (mdmunir).
...
- Chg: Remove `backend\modules\rbac\Module::allowActions`. Set access control directly with `backend\modules\rbac\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `backend\modules\rbac\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
