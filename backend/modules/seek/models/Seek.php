...
/**
 * This is the model class for table "{{%seek}}".
 *
 * @property string $id
 ...
 */
class Seek extends \yii\db\ActiveRecord
{
    const TYPE_PUBLIC = 0;
    ...
    {
        return Yii::$app->db
            ->createCommand("SELECT count(*) FROM {{%seeks}}  WHERE album_id={$this->id}")
            ->queryScalar();
    }
    ...
    public static function getCoverPhoto($id = null, $cover_id = null)
    {
        if ($id == null) {
            if ($this->cover_id == self::COVER_NONE) {
                ...
                $path = Yii::$app->db
                ->createCommand('SELECT path FROM {{%seeks}} WHERE id='.$this->cover_id)
                ->queryScalar();
            }
        } else {
            if ($cover_id == self::COVER_NONE) {
                $path = Yii::$app->db
                    ->createCommand('SELECT path FROM {{%seeks}} WHERE album_id='.$id)
                    ->queryScalar();
                if (empty($path)) {
                ...
                $path = Yii::$app->db
                    ->createCommand('SELECT path FROM {{%seeks}} WHERE id='.$cover_id)
                    ->queryScalar();
            }
        }
        return $path;
    ...
    {
        $config = [
            'savePath' => Yii::getAlias('@frontend').'/web/uploads/user/images/', //存储文件夹
            'maxSize' => 2048 ,//允许的文件最大尺寸，单位KB
            'allowFiles' => ['.gif' , '.png' , '.jpg' , '.jpeg' , '.bmp'],  //允许的文件格式
        ];
        $up = new Uploader("file", $config, 'seek'.$this->id);
        $info = $up->getFileInfo();

        //存入数据库
        Yii::$app->db->createCommand()->insert('{{%seeks}}', [
            'name' => $this->name,
            'path' => 'http://localhost:81'.Yii::getAlias('@web').'/uploads/user/images/' . $this->created_by . '/' . $info['name'], //存储路径
